...
public class BinaryTreeRenderer extends JPanel {
    ...
    protected void paintComponent(Graphics g) {
        ...
        Map<TreeNode<Integer>, Point> positions = new HashMap<>();
        int totalW = layout(root, 0, positions);
        int startX = (getWidth() - totalW) / 2 + RADIUS;

        Map<Integer, Point> lines = new HashMap<>();
        for (Map.Entry<TreeNode<Integer>, Point> e : positions.entrySet()) {
            TreeNode<Integer> node = e.getKey();
            Point p = e.getValue();
            p.x = p.x + startX;
            if (node.left != null) lines.put(node.left.hashCode(), p);
            if (node.right != null) lines.put(node.right.hashCode(), p);
        }

        for (Map.Entry<TreeNode<Integer>, Point> e : positions.entrySet()) {
            TreeNode<Integer> node = e.getKey();
            Point p = e.getValue();
            Point parentP = lines.get(node.hashCode());
            if (parentP != null) {
    ...
    private int layout(TreeNode<Integer> node, int depth, Map<TreeNode<Integer>, Point> positions) {
